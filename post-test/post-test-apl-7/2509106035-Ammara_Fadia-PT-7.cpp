...
void tambahKursus(Kursus *kursus, int *jumlahKursus) {
    ...
    try {
        if (k.id.empty() || k.nama.empty() || k.instruktur.empty()) {
            throw "Semua data harus diisi!";
        }
...
void linearSearchNama(Kursus kursus[], int n, string cari) {
    tampilHeader("Linear Search Nama Kursus");

    bool ditemukan = false;

    for (int i = 0; i < n; i++) {
        cout << "Cek index ke-" << i << " : " << kursus[i].nama << endl;

        if (kursus[i].nama == cari) {
            cout << "Ditemukan:\n";
            tampilKursusPointer(&kursus[i]);
            ditemukan = true;
        }
...
int binarySearchID(Kursus kursus[], int n, string cari) {
    tampilHeader("Binary Search ID");
    ...
    while (kiri <= kanan) {
        ...
        cout << "Cek tengah index " << tengah
             << " : " << kursus[tengah].id << endl;

        if (kursus[tengah].id == cari) {
            return tengah;
        } else if (kursus[tengah].id < cari) {
            kiri = tengah + 1;
        } else {
            kanan = tengah - 1;
        }
...
void sortIDAsc(Kursus kursus[], int n) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            if (kursus[j].id > kursus[j + 1].id) {
                Kursus temp = kursus[j];
                kursus[j] = kursus[j + 1];
                kursus[j + 1] = temp;
            }
...
void menuAdmin(Kursus kursus[], int &jumlahKursus) {
    ...
    do {
        ...
        try {
            ...
            cout << "8. Search Nama Kursus (Linear)\n";
            cout << "9. Search ID (Binary)\n";
            cout << "10. Logout\n";
            cout << "Pilih: ";
            cin >> menu;
        ...
        switch (menu) {
            ...
            case 8: {
                ...
                cin.ignore();
                getline(cin, cari);
                linearSearchNama(kursus, jumlahKursus, cari);
                break;
            }
            ...
            case 9: {
                ...
                sortIDAsc(kursus, jumlahKursus);
                int hasil = binarySearchID(kursus, jumlahKursus, cari);

                if (hasil != -1) {
                    cout << "Ditemukan:\n";
                    tampilKursusPointer(&kursus[hasil]);
                } else {
                    cout << "Data tidak ditemukan\n";
                }
                break;
    ...
    while (true) {
        ...
        try {
            ...
            if (username.empty() || password.empty()) {
                throw "Username/Password tidak boleh kosong!";
            }
